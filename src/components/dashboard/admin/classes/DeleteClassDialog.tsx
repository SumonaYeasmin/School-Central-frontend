...
export function DeleteClassDialog({
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      setError(null);
      await onConfirm(classItem.id);
      onClose();
    } catch (err: any) {
      console.error("Failed to delete class:", err);
      setError(err?.message || "Failed to delete class. Please make sure no active students are in this class.");
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden rounded-3xl border border-slate-200/90 shadow-2xl bg-white flex flex-col">
        ...
        <div className="p-6 space-y-4 text-xs text-slate-600">
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1.5">
            <p className="text-slate-500 font-medium">Class to remove:</p>
            <div className="flex items-center justify-between">
              <span className="font-bold text-slate-900 text-base">{classItem.name}</span>
              {classItem.sections && classItem.sections.length > 0 && (
                <span className="text-[11px] font-semibold bg-slate-200 text-slate-700 px-2.5 py-0.5 rounded-md">
          ...
          <p className="text-slate-500 leading-relaxed">
            This will permanently remove the class and its sections.
          </p>
        </div>
